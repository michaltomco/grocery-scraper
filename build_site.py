...
def write_product_pages(products, nutrition, exact_page_slugs=None):
    ...
    for product_index, product in enumerate(product_keys):
        ...
        def _price_cell(e: dict) -> str:
            val = e.get("val")
            if val is None:
                return "-"
            return f"{val:.2f} / {e.get('unit', '')}"
        ...
        discount_rows = "".join(
            (
                ...
                f'<td class="discount-price">{esc(_price_cell(e))}</td>'
...
.day {{ width: 18px; height: 18px; border-radius: 4px; background: var(--track); opacity: .28; cursor: pointer;
  display: flex; align-items: center; justify-content: center; font-size: 10px; line-height: 1; font-weight: 400; color: var(--logo-ink); }}
.day.active {{ background: var(--store-color); opacity: 1; font-weight: 400; }}
...
.spark {{ width: 130px; vertical-align: middle; }}
...
#t tr.search-hidden {{ display:none !important; }}
.timeline-head {{ display: flex; flex-direction: column; gap: 3px; }}
